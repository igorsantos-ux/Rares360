...
export class PatientController {
    static async list(req, res) {
        try {
            ...
            const { status, aniversario, ltvMin, ltvMax, lastVisit: lastVisitFilter, origem, search } = req.query;
            ...
            if (ltvMin || ltvMax) {
                where.cachedLtv = {};
                if (ltvMin)
                    where.cachedLtv.gte = parseFloat(String(ltvMin));
                if (ltvMax)
                    where.cachedLtv.lte = parseFloat(String(ltvMax));
            }
            // 5. Filtro de Origem
            if (origem)
                where.leadSource = String(origem);
            ...
            const analyticalPatients = filteredPatients.map((p) => {
                const totalSpent = p.cachedLtv || 0;
                const lastVisit = p.lastVisit || null;
                ...
                return {
                    ...
                    profitabilityMargin: p.profitabilityMargin || 0,
                    visitCount: p.visitCount || 0,
                    averageTicket: p.averageTicket || 0,
                };
            });
            const now = new Date();
