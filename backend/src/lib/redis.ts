...
function createRedisClient(): any {
    ...
    try {
        const client = new (RedisLib as any)(redisUrl, {
...
export async function invalidateCache(clinicId: string, ...prefixes: string[]): Promise<void> {
...
export async function getRedisHealth(): Promise<{ status: string; latency_ms: number }> {
