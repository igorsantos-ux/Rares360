...
export class AnalyticsService {
    static async getDashboardInsights(clinicId) {
        // 1. Procedimento mais rentável
        const transactions = await prisma.transaction.findMany({
            where: { clinicId, type: 'INCOME', NOT: { procedureName: null } }
        });
        const procedureStats = transactions.reduce((acc, t) => {
            const name = t.procedureName;
            if (!acc[name])
                acc[name] = { name, revenue: 0, cost: 0, count: 0 };
        ...
        const leadBySource = leads.reduce((acc, l) => {
            acc[l.source] = (acc[l.source] || 0) + 1;
            return acc;
        }, {});
        const leadByStatus = leads.reduce((acc, l) => {
