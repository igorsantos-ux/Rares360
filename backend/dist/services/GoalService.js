...
export class GoalService {
    static getMonthYearKey(date = new Date()) {
        const m = (date.getMonth() + 1).toString().padStart(2, '0');
        const y = date.getFullYear();
        return `${m}-${y}`;
    }
    static getBusinessDaysPassed(start, end) {
        let count = 0;
    ...
    static async getMonthlyGoals(clinicId, monthYear) {
        return await prisma.monthlyGoal.findMany({
            where: { clinicId, monthYear },
            orderBy: [{ isPrimary: 'desc' }, { createdAt: 'asc' }]
        });
    }
    static async saveGoal(clinicId, data) {
    ...
    static async deleteGoal(clinicId, goalId) {
        const goal = await prisma.monthlyGoal.findUnique({ where: { id: goalId } });
        if (!goal || goal.clinicId !== clinicId)
            throw new Error('Meta não encontrada');
        if (goal.isPrimary)
            throw new Error('Não é possível excluir a meta principal do ciclo');
        return await prisma.monthlyGoal.delete({ where: { id: goalId } });
    }
    static async getGoalsReport(clinicId) {
        const now = new Date();
        const monthYear = this.getMonthYearKey(now);
