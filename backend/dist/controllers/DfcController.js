...
export class DfcController {
    static async getReport(req, res) {
        try {
            ...
            if (!clinicId)
                return res.status(403).json({ error: 'Clínica não informada no contexto.' });
            const mockDfc = {
                kpis: {
    ...
    static async getProjection(req, res) {
    ...
    static async simulateScenarios(req, res) {
        try {
            const clinicId = tenantContext.getStore()?.clinicId;
            if (!clinicId)
                return res.status(403).json({ error: 'Clínica não informada.' });
