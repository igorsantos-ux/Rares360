...
export class HistoryService {
    static async getYearlySummary(year = 2026, clinicId) {
    ...
    static async getDetailedProcedures(clinicId) {
    ...
    static async getWeeklyAnalysis(month, clinicId, year = 2026) {
        ...
        transactions.forEach(t => {
            ...
            if (day <= 7)
                weeklyData[0].revenue += t.amount;
            else if (day <= 14)
                weeklyData[1].revenue += t.amount;
            else if (day <= 21)
                weeklyData[2].revenue += t.amount;
            else
                weeklyData[3].revenue += t.amount;
        });
        return weeklyData;
    }
}
